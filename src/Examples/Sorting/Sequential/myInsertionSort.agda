...
module Examples.Sorting.Sequential.myInsertionSort (M : Comparable) where
...
insert x (y ∷ ys) (h ∷ hs) = 
  ...
  (λ x≰y → 
    bind (F _) (insert x ys hs) 
      (λ (x∷ys' , x∷ys↭x∷ys' , sorted-x∷ys') → 
      ret ( (y ∷ x∷ys') , 
            trans (swap x y (↭-reflexive refl)) (prep y x∷ys↭x∷ys') , 
